...
OUTPUT_ROOT = os.path.join(EXPERIMENTS_DIR, "output")
...
def write_file(file_path: str, content: str, description: str = "Update code"):
    ...
    try:
        ...
        # 强制阻断散落根目录
        # 计算相对于 OUTPUT_ROOT 的路径
        try:
            rel_path = os.path.relpath(target_path, OUTPUT_ROOT)
        except ValueError:
             return f"❌ REJECTION: 路径错误！文件 '{file_path}' 不在 output 目录下。"

        parts = rel_path.split(os.sep)
        # parts[0] 是项目名 (snake_game)，parts[1] 是文件名 (main.py)
        # 所以至少需要 2 部分，且不能是 . (即 output 根目录)
        ...
        clean_relative_path = rel_path.replace("\\", "/")
